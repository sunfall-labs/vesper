...
//
// `pnpm verify` is what CI runs and what a contributor runs before opening a
// pull request. Everything it does is also runnable on its own — `pnpm build`,
// `pnpm lint`, and so on — so a failure here is always reproducible with one
// shorter command, which is printed alongside it.
//
...
/**
 * Run one pnpm script, buffering its output.
 *
      ...
      reject(
        new Error(
          `${label} failed with exit code ${code}. ` +
            `Reproduce with: ${pnpmCommand} ${args.join(' ')}`,
        ),
      );
    });
  });
...
const runAll = async (label, tasks, concurrency) => {
  ...
  const worker = async () => {
    for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
      try {
        await run(next.label, next.args);
      } catch (error) {
...
const main = async () => {
  ...
  await runAll(
    'source static gates',
    [
      { label: 'workspace format check', args: ['format:check'] },
      { label: 'workspace lint', args: ['lint'] },
      { label: 'workspace typecheck', args: ['typecheck'] },
    ],
    concurrency,
  );
