...
// get at the interface. Both are also public entry points — `./compaction` and
// `./context-window` — so the split is already load-bearing for callers.

export interface Policy {
  /**
   * Headroom to leave below the context window. Compaction fires when the
   * estimate crosses `contextWindow - reserveTokens`.
   */
  readonly reserveTokens: number;
  /** Recent tokens preserved unsummarized. */
  readonly keepRecentTokens: number;
  /** Instruction given to the model when producing the summary. */
  readonly instructions: string;
  /**
   * The system prompt the summarization call runs under.
   *
   * Distinct from {@link instructions}, which says *what* to capture. This
   * says what the call is at all: read a conversation and describe it, rather
   * than continue it. That is not a hypothetical failure — every message in
   * front of the instruction is a conversation asking to be continued, and a
   * model handed one without this will answer the last question in it and
   * return that as the summary.
   *
   * Optional, defaulting to {@link defaultSystem}.
   */
  readonly system?: string | undefined;
  /**
   * The model's context window, which is what makes the proactive trigger
   * possible at all.
   *
   * Optional, and undefined by default, because the loop targets the
   * `LanguageModel` tag and that tag does not carry a window — only the
   * provider adapter knows one, and asking it would be the import this family
   * exists to avoid. A caller that knows which model it wired can say so, and
   * gets compaction *before* an overflow instead of after it. A caller that
   * does not still gets the reactive path, which is the one that saves runs.
   */
...
export const compact = Effect.fn('Agent.compact')(function* (
  ...
  const summary = yield* LanguageModel.generateText({
    prompt: Prompt.make([
      ...
      { role: 'system', content: policy.system ?? defaultSystem },
      ...split.older,
